...
# Eye-only deviation thresholds (degrees)
# Lower values make eye-only "looking away" easier to trigger
EYE_YAW_THRESHOLD_DEG = 8
EYE_PITCH_THRESHOLD_DEG = 6
...
screen_position_file = os.path.join(data_dir, "screen_position.txt")
...
def get_yaw_pitch_from_vector(direction_vec):
    """
    Calculates yaw and pitch (in degrees) from a normalized 3D direction vector,
    relative to a local (0,0,-1) forward direction (negative Z).
    - Yaw: angle around local Y-axis. Negative for left, positive for right.
    - Pitch: angle around local X-axis. Negative for up, positive for down.
    """
    # Ensure vector is normalized
    direction_vec = _normalize(direction_vec)

    # Calculate Yaw (angle around Y-axis, in XZ plane)
    xz_proj = np.array([direction_vec[0], 0, direction_vec[2]])
    if np.linalg.norm(xz_proj) < 1e-9: # Handle cases where vector is perfectly vertical
        yaw_rad = 0.0
    else:
        xz_proj = _normalize(xz_proj)
        # Angle with (0,0,-1) vector
        yaw_rad = math.acos(np.clip(np.dot(xz_proj, np.array([0,0,-1])), -1.0, 1.0))
        # Determine sign based on X component (looking left implies negative X)
        if direction_vec[0] < 0: # If x is negative, it's to the left
            yaw_rad = -yaw_rad
    
    # Calculate Pitch (angle around X-axis, in YZ plane)
    yz_proj = np.array([0, direction_vec[1], direction_vec[2]])
    if np.linalg.norm(yz_proj) < 1e-9: # Handle cases where vector is perfectly horizontal
        pitch_rad = 0.0
    else:
        yz_proj = _normalize(yz_proj)
        # Angle with (0,0,-1) vector
        pitch_rad = math.acos(np.clip(np.dot(yz_proj, np.array([0,0,-1])), -1.0, 1.0))
        # Determine sign based on Y component (looking up implies positive Y)
        if direction_vec[1] > 0: # If y is positive, it's upwards
            pitch_rad = -pitch_rad # Convention: positive pitch is looking down
...
def convert_gaze_to_screen_coordinates(combined_gaze_direction, calibration_offset_yaw, calibration_offset_pitch):
    ...
    # At this point:
    #   yaw_deg   > 0  => looking right,  < 0 => left
    #   pitch_deg > 0  => looking down,  < 0 => up
    raw_yaw_deg = yaw_deg
    raw_pitch_deg = pitch_deg

    
    # Specify degrees at which screen border will be reached
    # (symmetric range: -yawDegrees..+yawDegrees, -pitchDegrees..+pitchDegrees)
    yawDegrees = 5 * 6       # ~30 degrees left or right
    pitchDegrees = 2.0 * 10  # ~20 degrees up or down
    ...
    #   yaw_deg   = -yawDegrees => x=0 (far left)
    #   yaw_deg   = +yawDegrees => x=MONITOR_WIDTH (far right)
    #   pitch_deg = -pitchDegrees => y=0 (top)
    #   pitch_deg = +pitchDegrees => y=MONITOR_HEIGHT (bottom)
    unclamped_screen_x = int(yaw_deg * MONITOR_WIDTH / yawDegrees + MONITOR_WIDTH / 2)
    unclamped_screen_y = int(pitch_deg * MONITOR_HEIGHT / pitchDegrees + MONITOR_HEIGHT / 2)

    gaze_status = "Looking Forward"
    if unclamped_screen_x < 0:
        gaze_status = "Looking Left"
    elif unclamped_screen_x > MONITOR_WIDTH:
        gaze_status = "Looking Right"
    elif unclamped_screen_y < 0 or unclamped_screen_y > MONITOR_HEIGHT:
        gaze_status = "Looking Away"

    # Clamp screen position to monitor bounds
    screen_x = max(10, min(unclamped_screen_x, MONITOR_WIDTH - 10))
    screen_y = max(10, min(unclamped_screen_y, MONITOR_HEIGHT - 10))

    return screen_x, screen_y, raw_yaw_deg, raw_pitch_deg, gaze_status
